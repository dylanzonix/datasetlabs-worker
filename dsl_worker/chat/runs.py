...
def _charge_run_credits(
    db, user_id, spend_cents: int, project_id, reason: str, external_ref=None
) -> None:
    ...
    `external_ref` tags the ledger entries with the originating run id so
    the per-turn coin can sum them back out (see run_cost_usd_from_ledger).
    """
    if spend_cents <= 0:
        return
...
def run_cost_usd_from_ledger(db, run_id) -> float:
    ...
    total_cents = (
        ...
        .filter(BalanceLedger.external_ref == str(run_id))
        .scalar()
    )
...
def _force_persist_terminal(
    *,
    ...
    try:
        ...
        _charge_run_credits(
            fresh, user_id, residual_cents, project_id, reason="chat_run", external_ref=str(run_id)
        )
        fresh.commit()
        fresh.refresh(run)
...
async def _drive_agent(
    ...
    try:
        ...
        def _persist_assistant_incremental(ldb_local) -> None:
            ...
            try:
                ...
                ac: Dict[str, Any] = {
                    ...
                    # the end-of-turn applied_changes build. The coin is
                    # the LEDGER sum for this run (chat_run + the table
                    # scrape / apify drain we tag with the run id) — not
                    ...
                    if delta_cents > 0:
                        try:
                            _charge_run_credits(
                                ...
                                reason="chat_run", external_ref=str(run_id),
                            )
                            ldb.commit()
        ...
        try:
            ...
            # reflects the full turn cost before we read the coin back out
            # of it (incremental cost_update charges already debited
            # charged_cents_ref["value"] mid-turn).
            residual_cents = spend_cents - charged_cents_ref["value"]
            _charge_run_credits(
                db, user_id, residual_cents, project_id, reason="chat_run", external_ref=str(run_id)
            )
