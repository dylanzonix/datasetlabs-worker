"""Shared chat-run state: event bus, persistence, lifecycle transitions.

A `ChatRun` row carries the state machine (queued → running → paused
→ completed | failed | cancelled). Events are persisted to
`ChatRunEvent` so reconnecting clients can replay from a cursor.

This module owns the infrastructure pieces used by `chat/runs.py`
(the chat turn lifecycle that actually starts runs):

- `_RunBus`              in-process pubsub for live subscribers.
- `emit_event` / `emit_events_batch`
                        persist a ChatRunEvent and fan out to subs.
- `publish_*_delta`     live-only deltas (no DB) for streaming
                        token / thinking content between checkpoints.
- `tail_events()`       async-generates events for a subscriber: first
                        replays persisted events from the cursor,
                        then tails new ones via the bus until the run
                        reaches a terminal status.
- `mark_run_*` / `_mark_run_failed`
                        terminal status transitions.
- `request_pause` / `request_cancel` / `resume_run`
                        pause/cancel signaling + resume-as-new-run.
- `recover_orphan_runs` / `orphan_recovery_loop`
                        marks runs failed when their owning task dies.
"""
...
def recover_orphan_runs() -> int:
    ...
    # Staleness threshold: chat emits a heartbeat event every 30s
    # while the run is alive (see chat.runs._heartbeat_loop). A
    ...
    try:
        ...
        for run_id in flipped_ids:
            ...
            try:
                from dsl_worker.chat.runs import cancel_run as _cancel_v2
                _cancel_v2(run.id)
            except Exception:
