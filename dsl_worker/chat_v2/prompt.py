...
- **`browser_use`** — comprehensive reader for **one specific site**. Use when (a) the user's ask is "everything on `<site>`" or implies it, (b) no Apify actor covers that site, and (c) web_harvest would only get whatever's indexed (which is partial — fine for "examples of," not fine for "every"). BU navigates the site like a person — pagination, cohort/tab clicks, expanding rows. Bounded by task scope on one site, not item count. Slower and costlier per row than APIs; don't use when an apify actor or API already covers the same site.
